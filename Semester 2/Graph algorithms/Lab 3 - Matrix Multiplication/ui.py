...
class UI:
    ...
    def find_lowest_cost_walk(self):
        ...
        vertices_list = self.__available_graphs[self.__current_graph].vertices_iterator()
        if source_vertex not in vertices_list or destination_vertex not in vertices_list:
            raise VertexException("The mentioned vertices must be contained by the current graph!")

        matrix = matrix_multiplication_algorithm(self.__available_graphs[self.__current_graph])
        cost = matrix[source_vertex][destination_vertex]

        if str(cost) == 'inf':
            raise PathException("There is no shortest path between the mentioned vertices.")

        print(f"\nThe walk from {source_vertex} to {destination_vertex} has the minimum cost {cost}.")

        print_intermediate_matrices(self.__available_graphs[self.__current_graph])

        print(f"\nThe path: {get_shortest_walk(self.__available_graphs[self.__current_graph], source_vertex, destination_vertex)}.")
    ...
    def print_menu():
        print("\n"
              ...
              "[21] Find lowest cost walk(matrix multiplication algorithm).\n"
              "[0] Exit."
              )
    ...
    def start_program(self):
        ...
        while True:
            ...
            except PathException as cycle_error:
                print(cycle_error)
