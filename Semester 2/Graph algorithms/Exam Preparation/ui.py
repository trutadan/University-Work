...
class UI:
    ...
    def bfs_ui(self):
        ...
        print(f"BFS: {bfs(self.__available_graphs[self.__current_graph], source_vertex)}.")
    ...
    def dfs_ui(self):
        ...
        print(f"DFS: {dfs(self.__available_graphs[self.__current_graph], source_vertex)}.")

    def minimum_length_path_ui(self):
        source_vertex = input("\nEnter the source vertex: ")
        if source_vertex.isnumeric():
            source_vertex = int(source_vertex)
        else:
        ...
        paths_length, previous = minimum_length_path(self.__available_graphs[self.__current_graph], source_vertex)
        if destination_vertex not in paths_length:
            print("There is no path between the given vertices!")

        print(f"Minimum length path starting from {source_vertex} to {destination_vertex} is {paths_length[destination_vertex]}.")

        path = list()
        each_vertex = destination_vertex
        while each_vertex is not source_vertex:
            path.append(each_vertex)
            each_vertex = previous[each_vertex]
    ...
    def find_connected_components_ui(self):
        components = find_connected_components(self.__available_graphs[self.__current_graph])
    ...
    def find_minimum_cost_walk_dijkstra(self):
        source_vertex = input("\nEnter the source vertex: ")
        if source_vertex.isnumeric():
            source_vertex = int(source_vertex)
        else:
        ...
        distances, previous = dijkstra_minimum_cost(self.__available_graphs[self.__current_graph], source_vertex)

        print(f"The cost from {source_vertex} to {destination_vertex} is: {distances[destination_vertex]}.")
    ...
    def find_minimum_cost_walk_bellman_fort(self):
        source_vertex = input("\nEnter the source vertex: ")
        if source_vertex.isnumeric():
            source_vertex = int(source_vertex)
        else:
        ...
        distances, previous = bellman_ford_minimum_cost(self.__available_graphs[self.__current_graph], source_vertex)
    ...
    def has_cycles_ui(self):
        cycles = find_all_cycles(self.__available_graphs[self.__current_graph])
        if not len(cycles):
    ...
    def is_tree_ui(self):
        if is_tree(self.__available_graphs[self.__current_graph]):
            print("Yes, it is a tree!")
            return

        print("No, it is not a tree!")

    def prim_minimum_spanning_tree_ui(self):
        tree_edges, tree_cost = prim_minimum_spanning_tree(self.__available_graphs[self.__current_graph])

        print(f"The minimum spanning tree: {tree_edges}")
        print(f"The cost: {tree_cost}")

    def kruskal_minimum_spanning_tree_ui(self):
        tree_edges, tree_cost = kruskal_minimum_spanning_tree(self.__available_graphs[self.__current_graph])
    ...
    def print_menu():
        print("\n"
              ...
              "[19] BFS.\n"
              "[20] DFS.\n"
              "[21] Minimum length path.\n"
              "[22] Find connected components.\n"
              "[23] Dijkstra minimum cost.\n"
              "[24] Bellman-Fort minimum cost.\n"
              "[25] Find if any cycles.\n"
              "[26] Is tree?\n"
              "[27] Prim minimum spanning tree.\n"
              "[28] Kruskal minimum spanning tree.\n"
              "[0] Exit."
              )
    ...
    def start_program(self):
        dictionary_with_commands = {"1": self.create_empty_undirected_graph, "2": self.create_random_undirected_graph,
                                    ...
                                    "19": self.bfs_ui, "20": self.dfs_ui, "21": self.minimum_length_path_ui,
                                    "22": self.find_connected_components_ui, "23": self.find_minimum_cost_walk_dijkstra,
                                    "24": self.find_minimum_cost_walk_bellman_fort, "25": self.has_cycles_ui,
                                    "26": self.is_tree_ui, "27": self.prim_minimum_spanning_tree_ui,
                                    "28": self.kruskal_minimum_spanning_tree_ui
                                    }
