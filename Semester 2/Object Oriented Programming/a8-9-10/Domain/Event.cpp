...
std::ostream& operator<<(std::ostream& writer, const Event& event)
{
    writer << event.get_title() << "|" << event.get_description() << "|" << event.get_datetime().date_to_string() << "|" << event.get_datetime().time_to_string() << "|" << event.get_number_of_people() << "|" << event.get_link();
    return writer;
}
