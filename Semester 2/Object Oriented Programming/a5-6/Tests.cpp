...
void test_getters__random_datetime_instance__successfully_working()
{
	...
	assert(datetime.datetime_to_string() == "17/4/2022, 13:26");
...
void test_to_string__random_datetime__correct_string()
{
	...
	assert(datetime.datetime_to_string() == "17/4/2022, 13:06");
	assert(datetime.date_to_string() == "2022/4/17");
...
{
	DynamicVector<Event>* dynamic_vector = new DynamicVector<Event>;
	Repository repo = Repository(dynamic_vector);
...
void test_repository_update__inexistent_title__throws_exception()
{
	...
	auto event_from_repo = repo.get_all_events()[0];
...
void test_repository_get_all_events__check_events__correct_events()
{
	...
	auto events = repo.get_all_events();
...
void test_remove_event_from_repository__existent_event__event_successfully_removed()
{
	...
	assert(controller->get_repository_length() == 0);
	assert(controller->get_removal_history_length() == 0);
	...
	assert(controller->get_repository_length() == 0);
	assert(controller->get_removal_history_length() == 1);
...
void test_update_event_from_repository__existent_event__event_successfully_updated()
{
	...
	auto updated_event = controller->get_all_events_from_repository()[0];
...
void test_get_all_unadded_events_for_given_month__different_cases_when_events_are_both_in_list_and_repository_or_not__successfully_working()
{
	...
	DynamicVector<Event>* filtered_event_for_month_one = controller->get_all_unadded_events_for_given_month(1);

	assert(filtered_event_for_month_one->get_length() == 2);

	controller->add_event_to_list(second_event);

	delete filtered_event_for_month_one;

	filtered_event_for_month_one = controller->get_all_unadded_events_for_given_month(1);

	assert(filtered_event_for_month_one->get_length() == 1);
...
void test_get_all_unadded_events__different_cases_when_events_are_both_in_list_and_repository_or_not__successfully_working()
{
	...
	DynamicVector<Event>* filtered_event_for_month_one = controller->get_all_unadded_events();

	assert(filtered_event_for_month_one->get_length() == 3);
	...
	assert(filtered_event_for_month_one->get_length() == 0);
...
void test_sort_events_by_date__different_dates_from_same_year__successfully_sorted()
{
	...
	Event* events = controller->get_all_events_from_repository();
	int length = controller->get_repository_length();

	controller->sort_events_by_date(events, length);
