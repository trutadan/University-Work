...
} // O(n) time complexity (where n<-number of elements)
...
int SortedMultiMap::findMinimumElement(int node)
{
	...
	return node;
} // O(h) <=> O(n) time complexity(where h <-height of the tree; n <-number of elements)

int SortedMultiMap::removeRecursive(int node, TElem element, bool& successfullyRemoved)
{
	if (node == -1)
		return -1;
	...
	if (this->elements[node].information == element) {
		...
		// the node to be removed has two descendants
			...
			this->elements[node].right = removeRecursive(this->elements[node].right, this->elements[minimumElement].information, successfullyRemoved);
			return node;
		}
	}
	...
	// the value of the element we are searching for is greater (in terms of relation) than the current one, we go left subtree
		...
		this->elements[node].left = removeRecursive(this->elements[node].left, element, successfullyRemoved);
		return node;
	}

	// the value of the element we are searching for is smaller (in terms of relation) than the current one, we go right subtree
	else {
		this->elements[node].right = removeRecursive(this->elements[node].right, element, successfullyRemoved);
		return node;
	}
} // O(h) <=> O(n) time complexity (where h<-height of the tree; n<-number of elements)
