...
class RentalRepository:
    def __init__(self):
        self._rental_data = IterableDataStructure()
...
class RentalJSONFileRepository(RentalRepository):
    ...
    def __read_all_from_file(self):
        with open(self.__file_path, 'r') as file:
            self._rental_data.clear()
            client_data_from_file = json.load(file)
            for field in client_data_from_file:
                self.add(Rental(field["rental_id"], field["movie_id"], field["client_id"], field["rent_date"], field["due_date"], field["return_date"]))

    def __write_all_to_file(self):
        with open(self.__file_path, 'w') as file:
            json.dump([rental.to_dictionary() for rental in self._rental_data], file, indent=4)

    def __append_to_file(self, rental):
        with open(self.__file_path, 'r+') as file:
            json.dump(rental.to_dictionary(), file, indent=4)
