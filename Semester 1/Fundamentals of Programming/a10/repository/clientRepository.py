...
class ClientRepository:
    def __init__(self):
        self._client_data = IterableDataStructure()
...
class ClientBinaryFileRepository(ClientRepository):
    ...
    def __read_clients_from_file(self):
        with open(self.__file_path, 'rb') as file:
            try:
                self._client_data = pickle.load(file)
            except EOFError:
                pass
...
class ClientJSONFileRepository(ClientRepository):
    ...
    def __read_all_from_file(self):
        with open(self.__file_path, 'r') as file:
            self._client_data.clear()
            for field in json.load(file):
                self._client_data[field["id"]] = (Client(field["id"], field["name"]))

    def __write_all_to_file(self):
        with open(self.__file_path, 'w') as file:
            client_list = [client.to_dictionary() for client in self._client_data]
            json.dump(client_list, file, indent=4)

    def __append_to_file(self, client):
        with open(self.__file_path, 'r+') as file:
            file_data = json.load(file)
            file_data.append(client.to_dictionary())
            file.seek(0)
