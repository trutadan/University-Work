...
class MovieRepository:
    def __init__(self):
        self._movie_data = IterableDataStructure()
...
class MovieJSONFileRepository(MovieRepository):
    def __init__(self, file_path):
        self.__file_path = file_path
        MovieRepository.__init__(self)
    ...
    def __read_all_from_file(self):
        with open(self.__file_path, 'r') as file:
            self._movie_data.clear()
            movie_data_from_file = json.load(file)
            for field in movie_data_from_file:
                self.add(Movie(field["id"], field["title"], field["description"], field["genre"]))

    def __write_all_to_file(self):
        with open(self.__file_path, 'w') as file:
            json.dump([movie.to_dictionary() for movie in self._movie_data], file, indent=4)

    def __append_to_file(self, movie):
        with open(self.__file_path, 'r+') as file:
            json.dump(movie.to_dictionary(), file, indent=4)
