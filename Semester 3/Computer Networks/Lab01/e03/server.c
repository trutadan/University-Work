...
int main() {
    ...
    while (1) {
        ...
        for (index = 0; index < stringLength / 2; index++) {
            char auxiliarCharacter = receivedString[index];
            receivedString[index] = receivedString[stringLength - index - 1];
            receivedString[stringLength - index - 1] = auxiliarCharacter;
        }

        res = send(c, (char*)&receivedString, sizeof(receivedString), 0);
        if (res != sizeof(receivedString))
            printf("Error sending resulted string!\n");
