...
def worker(cs):
    ...
    while not client_guessed:
        try:
            ...
            if client_number > server_number:
                server_message = 'smaller'
                cs.sendall(bytes(server_message, 'ascii'))

            if client_number < server_number:
                server_message = 'larger'
                cs.sendall(bytes(server_message, 'ascii'))
    ...
    if client_guessed:
        if threading.get_ident() == winner_thread:
            server_message = 'You win – within ' + str(number_of_tries) + ' tries!'
            cs.sendall(server_message.encode('utf8'))
            ...
            server_message = 'You lose – within ' + str(number_of_tries) + ' tries!'
            cs.sendall(server_message.encode('utf8'))
