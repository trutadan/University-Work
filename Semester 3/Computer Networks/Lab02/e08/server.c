...
uint16_t findCommonNumbers(uint16_t firstArray[], uint16_t firstArrayLength, uint16_t secondArray[], uint16_t secondArrayLength, uint16_t resultingArray[]) {
    uint16_t length = 0;

    for (uint16_t i = 0; i < firstArrayLength; i++)
        for (uint16_t j = 0; j < secondArrayLength; j++)
...
int main() {
    ...
    while (1) {
        ...
        uint16_t resultingArrayLength = findCommonNumbers(firstArray, firstArrayLength, secondArray, secondArrayLength, resultingArray);
