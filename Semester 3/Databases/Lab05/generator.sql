...
BEGIN
	SET @dateValue = DATEADD(DAY, RAND(CHECKSUM(NEWID())) * (1 + DATEDIFF(DAY, '2000-01-01', '2022-12-12')), '2000-01-01')
END
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				-- check if the current column has a foreign key property and if it has, 
				-- take its values from the referenced table
				IF EXISTS(
					SELECT *
